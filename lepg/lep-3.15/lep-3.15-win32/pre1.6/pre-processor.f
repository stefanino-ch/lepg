...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       write (*,*) "Version 1.6 ""Canigo"" (2021-01-17)"
       write (*,*) "Pere Casellas"
...
c      Do some mods over xk coefficient
       xk=1-xk
       if (xk.gt.1.) then
       xk=1.0
       end if
...
c      Set lineal coefficient 0 -> uniform 1 -> 0 wdth last cell
       xk_cells=2.*xk/float(ncells)

c      Set uniform cell width
       cuw=2.*xm/float(ncells)
...
c      Uniform with       
       coefg=xm/(float(nribss)*cuw-0.5*cuw)
       do i=1,nribss
       rib(i,22)=cuw*coefg
       end do

c      Modify using linear decrement in each cell
       do i=1,nribss
       rib(i,2)=cuw*float(i)-0.5*cuw
       rib(i,22)=rib(i,22)-xk_cells*rib(i,2)
       end do

c      Recompute x-position
       rib(1,2)=rib(1,22)/2.
       do i=2,nribss
       rib(i,2)=rib(i-1,2)+rib(i,22)
       end do

c      Normalize to total span
       coefg=xm/rib(nribss,2)
       do i=1,nribss
       rib(i,2)=rib(i,2)*coefg
       rib(i,22)=rib(i,22)*coefg
       end do
...
c      Modify using linear decrement in each cell
       do i=2,nribss
       rib(i,2)=cuw*float(i-1)
       rib(i,22)=cuw-xk_cells*rib(i,2)
       end do
...
c      First computes uniform rib distribution
c      rib(i,22) = width of cell i
       do i=1,nribss
       rib(i,22)=span/ncells
       end do

c      Width correction using chord coefficients
...
c      Local width correction local coefficient ci

       do i=1,nribss
       coefl=((chordmax-abs((rib(i,3)-rib(i,4))))*xk+
     + abs(rib(i,3)-rib(i,4)))/chordmax
       rib(i,22)=(span/ncells)*coefl
       end do       
...
c      Cell widths corrected by global coefficient
       do i=1,nribss
       rib(i,22)=rib(i,22)*coefg
       end do   
...
c     Detect if cells number is odd or even
      if (rib(1,22).lt.0.01) then
      evenodd=0
      else
      evenodd=1
      end if

c     Compute total cells number
      if (evenodd.eq.0) then
      ncells=(nribss-1)*2
      else
      ncells=2*nribss-1
...
c      rib(i,4) = y coordinate, trailing edge rib "i"
      ...
      write (23,'(A)') "Version 1.6 ""Canigo"" (2021-01-17)"
      write(23,'(A)')"*************************************************"
